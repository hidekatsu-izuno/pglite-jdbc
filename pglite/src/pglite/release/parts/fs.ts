...
export const createFS = ({
	...
	var FS: any = {
		...
		getStream: (fd: any) => FS.streams[fd],
		...
		major: (dev: any) => dev >> 8,
		minor: (dev: any) => dev & 255,
		makedev: (ma: any, mi: any) => (ma << 8) | mi,
		registerDevice(dev, ops) {
			FS.devices[dev] = { stream_ops: ops };
		},
		getDevice: (dev: any) => FS.devices[dev],
		getMounts(mount) {
			var mounts = [];
			var check = [mount];
			while (check.length) {
		...
		createDefaultDevices() {
			...
			FS.registerDevice(FS.makedev(1, 3), {
				read: () => 0,
				write: (stream: any, buffer: any, offset: any, length: any, pos: any) =>
					length,
				llseek: () => 0,
			});
		...
		createLazyFile(parent, name, url, canRead, canWrite) {
			class LazyUint8Array {
				...
				cacheLength() {
					...
					this.setDataGetter((chunkNum: any) => {
...
const createIDBFSInternal = ({ MEMFS }: any) => {
	...
	var IDBFS: any = {
		...
		getLocalSet: (mount: any, callback: any) => {
			var entries = {};
			function isRealDir(p: any) {
				return p !== "." && p !== "..";
			}
			function toAbsolute(root: any) {
				return (p: any) => PATH.join2(root, p);
			}
...
export const createFSRuntime = ({
	...
	var { NODEFS, bindNODEFSFS } = createNODEFSInternal({
		fs,
		mmapAlloc,
		HEAP8,
		ERRNO_CODES: ERRNO_CODES_MAP,
	});
	var FS: any = createFS({
		PATH_FS,
		MEMFS,
