...
import { intArrayFromString, UTF8ArrayToString } from "./text-codec.ts";
...
export const initializeRuntimeHost = async ({
	...
	if (ENVIRONMENT_IS_NODE) {
		...
		if (!importMetaUrl.startsWith("data:")) {
			scriptDirectory =
				nodePath.dirname(require("url").fileURLToPath(importMetaUrl)) + "/";
		}
...
export const createRuntimeHostFsSupport = ({
	...
	var { randomFill } = createRandomFill({
		ENVIRONMENT_IS_NODE,
		require,
		abort,
	});
	var PATH_FS = {
		resolve: (...args) => {
			...
			resolvedPath = PATH.normalizeArray(
				resolvedPath.split("/").filter((p: any) => !!p),
				!resolvedAbsolute,
			).join("/");
