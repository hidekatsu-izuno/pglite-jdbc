...
import { Parser as ProtocolParser, serialize } from "../pg-protocol";
import {
	type BackendMessage,
...
import { type DumpTarCompressionOptions, loadTar } from "./fs/tarUtils.js";
import type {
	DebugLevel,
	ExecProtocolOptions,
	ExecProtocolResult,
	...
	implements PGliteInterface, AsyncDisposable
{
	...
	static readonly MAX_BUFFER_SIZE: number = 2 ** 30;
		...
		let emscriptenOpts: Partial<PostgresMod> = {
			...
			instantiateWasm: (imports, successCallback) => {
				instantiateWasm(imports, options.wasmModule).then(
					({ instance, module }) => {
						// @ts-expect-error wrong type in Emscripten typings
	...
	async unlisten(
		...
		return this._runExclusiveListen(() =>
			this.#unlisten(channel, callback, tx),
		);
	}
