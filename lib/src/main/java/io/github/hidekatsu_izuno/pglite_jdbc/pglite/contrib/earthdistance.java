...
public final class earthdistance {
    public static final Extension<Object> earthdistance = new Extension<>();

    static {
        earthdistance.name = "earthdistance";
        earthdistance.setup = (_pg, emscriptenOpts, clientOnly) -> {
            var result = new ExtensionSetupResult<Object>();

            result.bundlePath = resource("earthdistance.tar.gz");
            return CompletableFuture.completedFuture(result);
        };
    }

    private static URL resource(String name) {
        var url = earthdistance.class.getClassLoader().getResource(name);
        if (url == null) {
