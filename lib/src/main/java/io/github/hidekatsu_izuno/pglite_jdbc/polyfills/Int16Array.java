...
public class Int16Array implements TypedArray {
    ...
    public Int16Array(ArrayBuffer array) {
    ...
    public Int16Array(ArrayBuffer array, int byteOffset, int length) {
    ...
    public short getInt16(int index) {
        ...
        var value = LITTLE_ENDIAN
            ? (bytes[base] & 0xFF) | ((bytes[base + 1] & 0xFF) << 8)
            : ((bytes[base] & 0xFF) << 8) | (bytes[base + 1] & 0xFF);
        return (short) value;
    }
    ...
    public void set(Int16Array source) {
        this.set(source, 0);
    }

    public void set(Int16Array source, int offset) {
    ...
    public void set(short[] source) {
        this.set(source, 0);
    }

    public void set(short[] source, int offset) {
        if (offset < 0 || offset > this.length) {
            throw new IndexOutOfBoundsException("offset out of range");
        }
        if (source.length + offset > this.length) {
        ...
        for (var i = 0; i < source.length; i++) {
            setInt16(offset + i, source[i]);
        }
    }

    public short[] toShortArray() {
        var copy = new short[this.length];
