...
public final class pg_buffercache {
    public static final Extension<Object> pg_buffercache = new Extension<>();

    static {
        pg_buffercache.name = "pg_buffercache";
        pg_buffercache.setup = (_pg, emscriptenOpts, clientOnly) -> {
            var result = new ExtensionSetupResult<Object>();

            result.bundlePath = resource("pg_buffercache.tar.gz");
            return CompletableFuture.completedFuture(result);
        };
    }

    private static URL resource(String name) {
        var url = pg_buffercache.class.getClassLoader().getResource(name);
        if (url == null) {
