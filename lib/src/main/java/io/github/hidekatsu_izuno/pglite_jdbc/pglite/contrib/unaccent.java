...
public final class unaccent {
    public static final Extension<Object> unaccent = new Extension<>();

    static {
        unaccent.name = "unaccent";
        unaccent.setup = (_pg, emscriptenOpts, clientOnly) -> {
            var result = new ExtensionSetupResult<Object>();

            result.bundlePath = resource("unaccent.tar.gz");
            return CompletableFuture.completedFuture(result);
        };
    }

    private static URL resource(String name) {
        var url = unaccent.class.getClassLoader().getResource(name);
        if (url == null) {
