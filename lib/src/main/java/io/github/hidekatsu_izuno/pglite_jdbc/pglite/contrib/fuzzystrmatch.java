...
public final class fuzzystrmatch {
    public static final Extension<Object> fuzzystrmatch = new Extension<>();

    static {
        fuzzystrmatch.name = "fuzzystrmatch";
        fuzzystrmatch.setup = (_pg, emscriptenOpts, clientOnly) -> {
            var result = new ExtensionSetupResult<Object>();

            result.bundlePath = resource("fuzzystrmatch.tar.gz");
            return CompletableFuture.completedFuture(result);
        };
    }

    private static URL resource(String name) {
        var url = fuzzystrmatch.class.getClassLoader().getResource(name);
        if (url == null) {
