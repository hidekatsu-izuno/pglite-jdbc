...
public final class amcheck {
    public static final Extension<Object> amcheck = new Extension<>();

    static {
        amcheck.name = "amcheck";
        amcheck.setup = (_pg, emscriptenOpts, clientOnly) -> {
            var result = new ExtensionSetupResult<Object>();

            result.bundlePath = resource("amcheck.tar.gz");
            return CompletableFuture.completedFuture(result);
        };
    }

    private static URL resource(String name) {
        var url = amcheck.class.getClassLoader().getResource(name);
        if (url == null) {
