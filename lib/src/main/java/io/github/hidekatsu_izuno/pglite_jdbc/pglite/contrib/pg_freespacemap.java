...
public final class pg_freespacemap {
    public static final Extension<Object> pg_freespacemap = new Extension<>();

    static {
        pg_freespacemap.name = "pg_freespacemap";
        pg_freespacemap.setup = (_pg, emscriptenOpts, clientOnly) -> {
            var result = new ExtensionSetupResult<Object>();

            result.bundlePath = resource("pg_freespacemap.tar.gz");
            return CompletableFuture.completedFuture(result);
        };
    }

    private static URL resource(String name) {
        var url = pg_freespacemap.class.getClassLoader().getResource(name);
        if (url == null) {
