...
public final class uuid_ossp {
    public static final Extension<Object> uuid_ossp = new Extension<>();

    static {
        uuid_ossp.name = "uuid-ossp";
        uuid_ossp.setup = (_pg, emscriptenOpts, clientOnly) -> {
            var result = new ExtensionSetupResult<Object>();

            result.bundlePath = resource("uuid-ossp.tar.gz");
            return CompletableFuture.completedFuture(result);
        };
    }

    private static URL resource(String name) {
        var url = uuid_ossp.class.getClassLoader().getResource(name);
        if (url == null) {
