...
public final class hstore {
    public static final Extension<Object> hstore = new Extension<>();

    static {
        hstore.name = "hstore";
        hstore.setup = (_pg, emscriptenOpts, clientOnly) -> {
            var result = new ExtensionSetupResult<Object>();

            result.bundlePath = resource("hstore.tar.gz");
            return CompletableFuture.completedFuture(result);
        };
    }

    private static URL resource(String name) {
        var url = hstore.class.getClassLoader().getResource(name);
        if (url == null) {
