...
public class Float32Array implements TypedArray {
    ...
    public Float32Array(ArrayBuffer array, int byteOffset, int length) {
    ...
    public void setFloat(int index, float value) {
        ...
        if (LITTLE_ENDIAN) {
            ...
            bytes[base + 3] = (byte) bits;
            bytes[base + 2] = (byte) (bits >> 8);
            bytes[base + 1] = (byte) (bits >> 16);
            bytes[base] = (byte) (bits >> 24);
        }
    }

    public void set(Float32Array source) {
        this.set(source, 0);
    }

    public void set(Float32Array source, int offset) {
    ...
    public void set(float[] source, int offset) {
        if (offset < 0 || offset > this.length) {
            throw new IndexOutOfBoundsException("offset out of range");
        }
        if (source.length + offset > this.length) {
        ...
        for (var i = 0; i < source.length; i++) {
            setFloat(offset + i, source[i]);
        }
    }

    public float[] toFloatArray() {
        var copy = new float[this.length];
