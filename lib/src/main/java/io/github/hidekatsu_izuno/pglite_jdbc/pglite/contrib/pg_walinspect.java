...
public final class pg_walinspect {
    public static final Extension<Object> pg_walinspect = new Extension<>();

    static {
        pg_walinspect.name = "pg_walinspect";
        pg_walinspect.setup = (_pg, emscriptenOpts, clientOnly) -> {
            var result = new ExtensionSetupResult<Object>();

            result.bundlePath = resource("pg_walinspect.tar.gz");
            return CompletableFuture.completedFuture(result);
        };
    }

    private static URL resource(String name) {
        var url = pg_walinspect.class.getClassLoader().getResource(name);
        if (url == null) {
